...
vlc_module_begin()
    ...
    add_integer         ("nysa-width", 320, T_WIDTH, LT_WIDTH, false      )
        change_private  (                                                 )
    add_integer         ("nysa-height", 200, T_HEIGHT, LT_HEIGHT, false   )
        change_private  (                                                 )
    add_integer         ("nysa-pitch", 640, T_PITCH, LT_PITCH, false      )
        change_private  (                                                 )
    add_string          ("nysa-chroma", "RV16", T_CHROMA, LT_CHROMA, true )
        change_private  (                                                 )
    add_obsolete_string ("nysa-lock"                                      ) /* obsoleted since 1.1.1 */
    add_obsolete_string ("nysa-unlock"                                    ) /* obsoleted since 1.1.1 */
    add_obsolete_string ("nysa-data"                                      ) /* obsoleted since 1.1.1 */
...
{
  //FILE *f = fopen("/home/cospan/sandbox/debug.txt", "w");
  ...
  //fclose(f);
    ...
    /* Get the callbacks */
    vlc_format_cb setup = var_InheritAddress(vd, "nysa-setup");

    sys->lock = var_InheritAddress(vd, "nysa-lock");
    if (sys->lock == NULL) {
        msg_Err(vd, "missing lock callback");
        free(sys);
        return VLC_EGENERIC;
    }
    sys->unlock = var_InheritAddress(vd, "nysa-unlock");
    sys->display = var_InheritAddress(vd, "nysa-display");
    sys->cleanup = var_InheritAddress(vd, "nysa-cleanup");
    sys->opaque = var_InheritAddress(vd, "nysa-data");
    sys->pool = NULL;
    ...
    if (!fmt.i_chroma) {
        msg_Err(vd, "nysa-chroma should be 4 characters long");
        free(sys);
        return VLC_EGENERIC;
    }
...
static picture_pool_t *Pool(vout_display_t *vd, unsigned count)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        for (unsigned i = 0; i < PICTURE_PLANE_MAX; i++) {
            /* nysa-lock is responsible for the allocation */
